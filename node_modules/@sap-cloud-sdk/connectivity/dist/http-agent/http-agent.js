 ...
 */
function trustStoreOptions(options, destination) {
    if (destination.trustStoreCertificate) {
        ...
        if (filetype.toLowerCase() !== 'pem') {
            logger.warn(`The provided truststore ${destination.trustStoreCertificate.name} is not in 'pem' format which is currently the only supported format. Trustore is ignored.`);
            return options;
        }
        const decodedCertificate = Buffer.from(destination.trustStoreCertificate.content, 'base64').toString('utf8');
        return {
            ...options,
